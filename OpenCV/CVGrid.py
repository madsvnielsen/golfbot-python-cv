...
class CVGrid:
    ...
    def __init__(self, grid_size, frame_size) -> None:
        ...
        self.cell_size = (
            int(frame_size[0]/grid_size[0]), int(frame_size[1]/grid_size[1]))
        self.cells = [[CVCell((y, x), (x*self.cell_size[0], y*self.cell_size[1]), "unblocked")
                ...
                if cell.status == "blocked":
                    for dy in range(-neighbors, neighbors + 1):
                        for dx in range(-neighbors, neighbors + 1):
                            ny, nx = cell.position[0] + \
                                dy, cell.position[1] + dx
    ...
    def navigate(self, destination, source):
        ...
        grid = [[1 for _ in range(self.grid_size[0])]
        ...
        while i < len(source):
            ...
            if i > 0 and (i + 1) % 6 == 0:  # Check for every sixth iteration (1-indexed)
                ...
                if i + 1 < len(destArr):
                    print(
                        f"Running A* search from {special_dest} to {destArr[i]}")
